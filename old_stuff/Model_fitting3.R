...
#1.1. Update response variable (May 2022) ======================================
...
### Megan McPhee recommends that I use the model that is within 2 AICc of the
#lowest AICc that also provides the most information for simplicity's sake. So,
#just interpret model #1 (bm1). Still hang onto model #2 so that you can use it
#in chapter 2 if necessary
summary(bm1) #Conspecific abundance is definitely not significant. Removing it
#would make bm2 your best model. Replace bm1 object with bm2
bm1 <- bm2
summary(bm1)
...
null_model <- glmer.nb(Avg_number_strays ~ (1|Year), data = f_scaled,
                       control = glmerControl(optimizer="bobyqa",
                                              optCtrl = list(maxfun = 1e6))) #null
#model is failing to converge still, not sure what to do about that
AICc(bm1, null_model) #null model is worse
lmtest::lrtest(bm1, null_model) #complex model is better
...
#plot(residuals(bm2, type = "deviance") ~ fitted(bm2))
...
outlieryes <- data.frame(deviance_resid = residuals(bm1, type = "deviance"),
                        pred = fitted(bm1)) #named "outlieryes" because in a
#previous version of this script I had removed an outlier (after section 6). That
...
#plot(residuals(bm2, type = "pearson") ~ fitted(bm2))
...
par(mfrow=c(1,2))
hist(residuals(bm1, type = "deviance"))
#hist(residuals(bm2, type = "deviance"))

#Some slight right-skew in histograms
qqnorm(residuals(bm1, type = "deviance"))
#qqnorm(residuals(bm2, type = "deviance"))
qqnorm(residuals(bm1, type = "pearson"))
#qqnorm(residuals(bm2, type = "pearson"))
...
temp(bm1, f_scaled) #no evidence of temporal autocorrelation
#temp(bm2, f_scaled) 
...
#the sub_f_scaled dataset vs full f_scaled dataset
sub_f_scaled <- f_scaled[f_scaled$Avg_number_strays < 40,] #40 is an approx.
#break in the data

#Top model 
mean_mae_bm1 <- vector(length = 10)
mae_bm1 <- vector(length = 500)
cross_val(f_scaled, bm1, mae_bm1, mean_mae_bm1) #for f_scaled (full dataset
#including large obs. values), the mean MAE is 10.99
cross_val(sub_f_scaled, bm1, mae_bm1, mean_mae_bm1) #for sub_f_scaled, the
#mean MAE is 6.31 

#Second best model
#mean_mae_bm2 <- vector(length = 10)
#mae_bm2 <- vector(length = 500)
#cross_val(f_scaled, bm2, mae_bm2, mean_mae_bm2) #for f_scaled (full dataset
#including large obs. values), the mean MAE is 11.00
#cross_val(sub_f_scaled, bm2, mae_bm2, mean_mae_bm2) #for sub_f_scaled, the
#mean MAE is 6.31
...
cross_val(f_scaled, null_model, mae_null, mean_mae_null) #for f_scaled (full dataset
#including large obs. values), the mean MAE is 14.89
cross_val(sub_f_scaled, null_model, mae_null, mean_mae_null) #for sub_f_scaled,
